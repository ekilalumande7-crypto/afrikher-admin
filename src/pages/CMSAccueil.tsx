...
import {
  ...
  ExternalLink, Image as ImageIcon, Eye, Monitor, Smartphone
} from 'lucide-react';
...
export default function CMSAccueil() {
  ...
  const ImageUploader = ({ configKey, label, currentUrl, aspectHint }: {
    configKey: string; label: string; currentUrl: string; aspectHint?: string;
  }) => (
            ...
            {activeSection === 'identite' && (
              <div>
                ...
                <FieldRow label="Logo" description="Format PNG transparent recommandé. Max 500KB.">
                  <ImageUploader configKey="logo_url" label="Logo" currentUrl={config.logo_url || ''} aspectHint="PNG transparent, 300x80px idéal" />
                </FieldRow>
                ...
                <FieldRow label="Slogan / Tagline" description="Texte court sous le nom du site.">
                  <input
                    ...
                    className="w-full max-w-lg px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              ...
              <div>
                ...
                <FieldRow label="Image de fond" description="L'image affichée en arrière-plan du hero. Format JPEG ou WebP, 1920x800px minimum.">
                  <div className="space-y-3">
                    <ImageUploader configKey="hero_image" label="Hero" currentUrl={config.hero_image || ''} aspectHint="JPEG/WebP, 1920x800px, max 2MB" />
                    <input
                      type="text"
                      value={config.hero_image || ''}
                      onChange={(e) => updateConfig('hero_image', e.target.value)}
                      className="w-full px-4 py-2 border border-gray-200 rounded-lg text-xs text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                ...
                <FieldRow label="Bouton principal" description="Le premier bouton, mis en avant avec un style rempli.">
                  <div className="space-y-3">
                    ...
                    {config.hero_cta1_text && (
                      <div>
                        <span className="text-xs text-gray-400 mr-3">Aperçu :</span>
                        <span className="inline-block px-5 py-2 border border-amber-600 text-amber-600 text-xs font-semibold tracking-wider uppercase">
                          {config.hero_cta1_text}
                        </span>
                      </div>
                    )}
                ...
                <FieldRow label="Bouton secondaire" description="Le second bouton, plus discret avec un style contour." noBorder>
                  <div className="space-y-3">
                    ...
                    {config.hero_cta2_text && (
                      <div>
                        <span className="text-xs text-gray-400 mr-3">Aperçu :</span>
                        <span className="inline-block px-5 py-2 border border-amber-600/40 text-amber-600/70 text-xs font-semibold tracking-wider uppercase">
                          {config.hero_cta2_text}
                        </span>
                      </div>
                    )}
            ...
            {activeSection === 'apropos' && (
              <div>
                ...
                <FieldRow label="Image" description="Image illustrant la section à propos." noBorder>
                  <ImageUploader configKey="about_image" label="À propos" currentUrl={config.about_image || ''} aspectHint="JPEG, 800x600px recommandé" />
                </FieldRow>
              </div>
            )}
            ...
            {activeSection === 'fondatrice' && (
              <div>
                ...
                <FieldRow label="Photo" description="Portrait de la fondatrice.">
                  <ImageUploader configKey="foundress_photo" label="Fondatrice" currentUrl={config.foundress_photo || ''} aspectHint="Portrait carré, 400x400px" />
                </FieldRow>
                ...
                <FieldRow label="Biographie" description="Texte de présentation de la fondatrice." noBorder>
                  <textarea
                    ...
                    rows={5}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
                    placeholder="Entrepreneure engagée, animée par la volonté de valoriser la puissance et l'expertise des femmes africaines..."
                  />
                </FieldRow>
              </div>
            )}
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                        config.hero_carousel_enabled === 'true' ? 'bg-blue-600' : 'bg-gray-300'
                      }`}
                    >
                      <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                        config.hero_carousel_enabled === 'true' ? 'translate-x-6' : 'translate-x-1'
                      }`} />
                    </button>
                  ...
                  <FieldRow label="Images du carousel" description="URLs des images, séparées par des virgules. Utilisez le bouton pour uploader.">
                    <div className="space-y-3">
                      <textarea
                        ...
                        rows={3}
                        className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
                      ...
                      <label className={`inline-flex items-center px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 cursor-pointer transition-colors ${uploading === 'hero_carousel' ? 'opacity-50' : ''}`}>
                        {uploading === 'hero_carousel' ? (
                          ...
                          <><Upload size={14} className="mr-2" /> Ajouter une image</>
                        )}
                        <input
                          type="file"
                          accept="image/*"
                ...
                <FieldRow label="Couleur principale" description="La couleur dorée utilisée comme accent sur tout le site." noBorder>
                  <div className="flex items-center gap-3">
                    ...
                    <input
                      ...
                      value={config.primary_color || '#C9A84C'}
                      onChange={(e) => updateConfig('primary_color', e.target.value)}
                      className="w-32 px-3 py-2 border border-gray-300 rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      placeholder="#C9A84C"
                    />
                  </div>
                </FieldRow>
      ...
      {showPreview && (
        <div className="mt-6 bg-white border border-gray-200 rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-6 py-3 border-b border-gray-200">
            ...
            <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-lg">
              <button
                ...
                className={`p-1.5 rounded transition-colors ${previewMode === 'desktop' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-400'}`}
              >
                <Monitor size={14} />
              </button>
              <button
                onClick={() => setPreviewMode('mobile')}
                className={`p-1.5 rounded transition-colors ${previewMode === 'mobile' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-400'}`}
              >
                <Smartphone size={14} />
              </button>
            </div>
          ...
          <div className="p-6 bg-gray-50">
            <div
              className={`bg-[#0A0A0A] rounded-lg overflow-hidden mx-auto relative ${previewMode === 'mobile' ? 'w-[375px]' : 'w-full'}`}
              style={{
                minHeight: previewMode === 'mobile' ? '500px' : '320px',
                backgroundImage: config.hero_image && !config.hero_image.startsWith('/images/') ? `url(${config.hero_image})` : undefined,
            ...
            >
              ...
              <div className={`relative z-10 p-8 flex flex-col justify-center h-full ${
                ...
                <h1 className={`font-serif font-bold text-[#C9A84C] tracking-wider ${previewMode === 'mobile' ? 'text-3xl mb-2' : 'text-5xl mb-3'}`}
                  style={{ fontFamily: 'Cormorant Garamond, Georgia, serif' }}>
                  {config.site_name || 'AFRIKHER'}
                </h1>
                <p className={`italic text-gray-300 mb-3 ${previewMode === 'mobile' ? 'text-sm' : 'text-base'}`}
                  style={{ fontFamily: 'Cormorant Garamond, Georgia, serif' }}>
                  {config.site_tagline || "L'élégance hors du commun."}
                </p>
                <h2 className={`font-serif text-[#C9A84C]/80 mb-2 ${previewMode === 'mobile' ? 'text-xl' : 'text-2xl'}`}>
                  {config.hero_title || 'Découvrez AFRIKHER'}
                </h2>
                <p className={`text-gray-400 mb-5 max-w-lg ${previewMode === 'mobile' ? 'text-xs' : 'text-sm'}`}>
                  {config.site_description || 'Le magazine premium dédié à la femme africaine entrepreneure.'}
                </p>
                <div className="flex items-center gap-3">
                  {config.hero_cta1_text && (
                    <span className={`border border-[#C9A84C] text-[#C9A84C] font-bold tracking-wider uppercase ${previewMode === 'mobile' ? 'px-4 py-2 text-[8px]' : 'px-5 py-2.5 text-[10px]'}`}>
                      {config.hero_cta1_text}
                    </span>
                  )}
                  {config.hero_cta2_text && (
                    <span className={`border border-[#C9A84C]/30 text-[#C9A84C]/70 font-bold tracking-wider uppercase ${previewMode === 'mobile' ? 'px-4 py-2 text-[8px]' : 'px-5 py-2.5 text-[10px]'}`}>
                      {config.hero_cta2_text}
                    </span>
                  )}
                </div>
